...
def add_shipment_db(vendor_id, order_date, ttn, address, client_name, phone, item_code, cod_price, qty, drop_price, status):
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (vendor_id, str(order_date), str(ttn), str(address), str(client_name), str(phone), str(item_code), float(cod_price), int(qty), float(drop_price), str(status).lower())
        )
        conn.commit()
...
# --- ИНТЕЛЛЕКТУАЛЬНЫЙ ПАРСЕР GOOGLE ТАБЛИЦ И EXCEL ---
def smart_parse_imported_df(df_raw):
    header_row_idx = None
    for idx, row in df_raw.iterrows():
        row_str = " ".join([str(val) for val in row.values]).lower()
        if any(k in row_str for k in ["ттн", "№ нп", "клієнт", "клиент", "адрес"]):
            header_row_idx = idx
            break
    ...
    for col in df_raw.columns:
        ...
        if any(x in c_clean for x in ["ттн", "№ нп"]):
            normalized_cols[col] = "ttn"
        elif any(x in c_clean for x in ["дата заказа", "дата"]):
            normalized_cols[col] = "order_date"
        elif "адрес" in c_clean:
            normalized_cols[col] = "address"
        elif any(x in c_clean for x in ["клієнт", "клиент"]):
            normalized_cols[col] = "client_name"
        elif any(x in c_clean for x in ["телеф", "телефон", "phone"]):
            normalized_cols[col] = "phone"
        elif any(x in c_clean for x in ["код товара", "код"]):
            normalized_cols[col] = "item_code"
        elif "наложка" in c_clean and "сумма" not in c_clean:
            normalized_cols[col] = "cod_price"
        elif any(x in c_clean for x in ["количе", "кол-во", "кол"]):
            normalized_cols[col] = "qty"
        elif any(x in c_clean for x in ["medley", "ваша ціна", "дроп", "вхід"]):
            normalized_cols[col] = "drop_price"
        elif "статус" in c_clean:
            normalized_cols[col] = "status"

    df_renamed = df_raw.rename(columns=normalized_cols)
    return df_renamed
...
# Создание демо-поставщика Миши из скрина, если база пустая
...
with tab_shipments:
    ...
    # --- НАСТРОЙКИ И ФАЙЛОВЫЕ ОПЕРАЦИИ (ЭКСПОРТ / ИМПОРТ GOOGLE SHEET) ---
    ...
    with col_exp2:
        with st.expander("📂 Экспорт и Импорт Google Таблицы (Excel / CSV)", expanded=False):
            ...
            st.markdown("---")
            st.markdown("**📤 Импорт:** Загрузить файл со скрина (Google Sheet / CSV / XLSX)")
            uploaded_file = st.file_uploader("Выберите файл таблички:", type=["csv", "xlsx"])
            
            if uploaded_file is not None:
                try:
                    if uploaded_file.name.endswith(".csv"):
                        imp_df_raw = pd.read_csv(uploaded_file, header=None)
                    else:
                        imp_df_raw = pd.read_excel(uploaded_file, header=None)

                    # Умное распознавание колонок из вашей таблицы
                    parsed_df = smart_parse_imported_df(imp_df_raw)

                    st.markdown("**Распознанные данные для импорта:**")
                    st.dataframe(parsed_df.head(5), use_container_width=True)
                    
                    if st.button("🚀 Импортировать всю табличку в базу", type="primary"):
                        count_added = 0
                        for _, row in parsed_df.iterrows():
                            i_ttn = str(row.get("ttn", "")).strip()
                            if not i_ttn or i_ttn.lower() in ["nan", "none", "0", "ттн", "т т н"]:
                                continue

                            i_date = str(row.get("order_date", date.today())).split(" ")[0]
                            i_addr = str(row.get("address", "")) if str(row.get("address", "")) != "nan" else ""
                            i_client = str(row.get("client_name", "")) if str(row.get("client_name", "")) != "nan" else ""
                            i_phone = str(row.get("phone", "")) if str(row.get("phone", "")) != "nan" else ""
                            i_code = str(row.get("item_code", "2101")) if str(row.get("item_code", "")) != "nan" else "2101"
                            
                            try:
                                i_cod = float(str(row.get("cod_price", 0)).replace(",", ".").replace(" ", ""))
                            except Exception:
                                i_cod = 0.0
                                
                            try:
                                i_qty = int(float(str(row.get("qty", 1)).replace(",", ".").replace(" ", "")))
                            except Exception:
                                i_qty = 1
                                
                            try:
                                i_drop = float(str(row.get("drop_price", 0)).replace(",", ".").replace(" ", ""))
                            except Exception:
                                i_drop = 0.0

                            i_status = str(row.get("status", "новий")).strip().lower()
                            if i_status not in ["отримано", "відмова", "в дорозі", "новий"]:
                                i_status = "новий"

                            add_shipment_db(v_id, i_date, i_ttn, i_addr, i_client, i_phone, i_code, i_cod, i_qty, i_drop, i_status)
                            count_added += 1
                        
                        st.success(f"Успешно импортировано {count_added} ТТН в базу поставщика '{v_name}'!")
                        st.rerun()
                except Exception as e:
